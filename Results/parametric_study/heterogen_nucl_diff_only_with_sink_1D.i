...
[Mesh]
  ...
  xmax = 10000 #X-direction domain size, for this file it is in nm
  # uniform_refine = 2
[]
...
[Materials]
  ...
  [./prob_homeg]
    ...
    function = 'if(psi>0.01,0,1)*if(cv<0,0,cv)*Ns*kb_h*T*time_scale*exp(-Em/(kb*T))*exp(-abs(dgstar)/kb/T)'
    # outputs = exodus
  [../]
  ...
  [./prob_heterog]
    ...
    function = 'if(psi>0.01,0,1)*if(cv<0,0,cv)*Ns*kb_h*T*time_scale*exp(-Em/(kb*T))*exp(-abs(S*dgstar)/kb/T)'
    # outputs = exodus
  [../]
