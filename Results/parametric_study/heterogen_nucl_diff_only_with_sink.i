...
[Materials]
  ...
  [./Material_properties] #Sets the free energy properties of the material
    type = GenericConstantMaterial
    prop_names =  'Ef   Ab    Bb eps  Av   cVeq D0         sink'
    prop_values = '0.52 270.5 32 0.01 1000 1    8.37707e-7 0'
    # outputs = 'exodus'
  ...
  [./dgvmatrix]
    ...
    function = 'Ab*(sqrt((if(cv<0,0,cv)-cBeq)^2 + eps^2) - eps) + 0.5*Bb*(if(cv<0,0,cv)-cBeq)^2'
    args = 'cv'
    # material_property_names = 'Av cBeq'
    # args = 'cv'
  ...
  [./fcbulk_slope]
    ...
    material_property_names = 'cBeq Ab Bb eps'
    args = 'cv'
    function = 'Ab*(if(cv<0,0,cv)/sqrt((if(cv<0,0,cv)-cBeq)^2+eps^2)) + Bb*if(cv<0,0,cv)'
    # material_property_names = 'Av cBeq'
    # args = 'cv'
...
[UserObjects]
  ...
  [./term]
    type = Terminator
    expression = 'max(rate > 0.1, max_cv > 1)'
