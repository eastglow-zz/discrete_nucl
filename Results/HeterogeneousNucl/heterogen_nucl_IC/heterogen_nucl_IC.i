...
[Materials]
  ...
  [./Mobility]
    ...
    constant_expressions = '1e-9 1e-6 0.89 8.6173303e-5 300 1.39*(1e-2/length_scale^2)*time_scale 5.586e24*length_scale^3'
    args = cv
  ...
  [./probability]
    ...
    constant_expressions = '1e-9         1e-6       8.6173303e-5 300 0.89   9.87e12'
    function = 'cv*(1-psi)*Ns*w*time_scale*exp(-Em/(kb*T))*exp(-abs(dgstar)/kb/T)'
    # function = '0.0001'
    outputs = exodus
  [../]
  ...
  [./cveq_temp_dep]
    type = ParsedMaterial
    constant_names =       'kb           T   Ef   Sf'
    constant_expressions = '8.6173303e-5 300 0.52 4.7'
