...
[Materials]
  ...
  [./prob_homeg]
    ...
    function = 'if(psi>0.001,0,1)*if(cv<0,0,1)*cv*Ns*kb_h*T*time_scale*exp(-Em/(kb*T))*exp(-abs(dgstar)/kb/T)'
    outputs = exodus
  [../]
  ...
  [./prob_heterog]
    ...
    function = 'if(psi>0.001,0,1)*if(cv<0,0,1)*cv*Ns*kb_h*T*time_scale*exp(-Em/(kb*T))*exp(-abs(S*dgstar)/kb/T)'
    outputs = exodus
  [../]
...
[Outputs]
  ...
  csv = true
  perf_graph = true
  file_base = 'equil_shift_diff_out'
[]
